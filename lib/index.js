...
module.exports = function (ops) {
  // Default ops
  ops = ops || {};
  // Setup (ops.bin only kept for backwards compatibility -- use a different version in package.json if desired)
  var bin = ops.bin || join(require.resolve('mocha'), '..', 'bin', (ops.coverage ? '_' : '') + 'mocha');
  var env = _.extend(_.clone(process.env), ops.env || {});
  var coverage = ops.coverage;

  ops = _.omit(ops, ['bin', 'env', 'coverage']);
