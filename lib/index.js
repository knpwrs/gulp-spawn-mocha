...
var _ = require('lodash'),
    ...
    dashify = _.compose(inflection.dasherize, inflection.underscore),
...
module.exports = function (ops, coverage) {
  ...
  var stream = through(function (file) {
    ...
    if (outstream) {
      that._child.stdout.on('data', function (data) {
        console.log(data.toString())
      });
      var s = /string/i.test(typeof outstream) ? fs.createWriteStream(outstream, {flags: 'a'}) : outstream;
...
function addArg(args, name, val) {
  ...
  args.push(name.length > 1 ? '--' + dashify(name) : '-' + name);
